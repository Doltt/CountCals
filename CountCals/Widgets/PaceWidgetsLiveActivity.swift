...
struct PaceWidgetsLiveActivity: Widget {
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: CountCalsAttributes.self) { context in
            // Lock screen UI
            LockScreenView(
                state: context.state,
...
private struct LockScreenView: View {
    ...
    var body: some View {
        HStack(alignment: .center) {
            ...
            Link(destination: URL(string: "countcals://add-food")!) {
                Text("Add")
...
extension CountCalsAttributes {
    fileprivate static var preview: CountCalsAttributes {
        CountCalsAttributes(dailyTarget: 2200)
    }
}

extension CountCalsAttributes.ContentState {
    fileprivate static var normal: CountCalsAttributes.ContentState {
        CountCalsAttributes.ContentState(
    ...
    fileprivate static var low: CountCalsAttributes.ContentState {
        CountCalsAttributes.ContentState(
...
#Preview("Notification", as: .content, using: CountCalsAttributes.preview) {
    PaceWidgetsLiveActivity()
} contentStates: {
    CountCalsAttributes.ContentState.normal
    CountCalsAttributes.ContentState.low
}
